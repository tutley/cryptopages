...
type (
	...
	// SearchUserCommand is the command line data structure for the search action of user
	SearchUserCommand struct {
		// Search by crypto coins accepted (abbreviated)
		Coins string
...
func (cmd *SearchUserCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.SearchUser(ctx, path, stringFlagVal("coins", cmd.Coins), stringFlagVal("jobCategory", cmd.JobCategory), stringFlagVal("skills", cmd.Skills))
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *SearchUserCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var coins string
	cc.Flags().StringVar(&cmd.Coins, "coins", coins, `Search by crypto coins accepted (abbreviated)`)
	var jobCategory string
	cc.Flags().StringVar(&cmd.JobCategory, "jobCategory", jobCategory, `Search by job Category`)
	var skills string
	cc.Flags().StringVar(&cmd.Skills, "skills", skills, `Search by Skills`)
}
...
func (cmd *UpdateUserCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	...
	cc.Flags().StringVar(&cmd.ContentType, "content", "", "Request content type override, e.g. 'application/x-www-form-urlencoded'")
	var username string
	cc.Flags().StringVar(&cmd.Username, "username", username, `The unique username for this person`)
}
